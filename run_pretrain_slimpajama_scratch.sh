...
export WANDB_RUN_NAME="${WANDB_RUN_NAME:-gemma-270m-slimpajama-scratch-$(date +%Y%m%d-%H%M)}"
export WANDB_HTTP_TIMEOUT=300
export WANDB_INIT_TIMEOUT=120
# ------------------------------------------------------------------

# Set CUDA_VISIBLE_DEVICES to target specific GPUs, e.g.:
#   CUDA_VISIBLE_DEVICES=0,1 bash run_pretrain_slimpajama_scratch.sh
...
mkdir -p ./checkpoints/gemma-slimpajama-scratch-inr
mkdir -p ./logs
...
torchrun \
  ...
  --output_dir "./checkpoints/gemma-slimpajama-scratch-inr" \
  \
  --do_train True \
  --do_eval True \
  \
