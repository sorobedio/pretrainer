#!/usr/bin/env bash
# Evaluate checkpoints/gemma-scratch on perplexity + benchmarks.
...
#
# Usage:
#   bash eval_gemma_scratch.sh                  # wikitext perplexity only
#   EVAL_MODE=all bash eval_gemma_scratch.sh    # wikitext + lambada + text8 + benchmarks

set -euo pipefail

CHECKPOINT_DIR="${CHECKPOINT_DIR:-./checkpoints/gemma-scratch}"
OUTPUT_PATH="${OUTPUT_PATH:-${CHECKPOINT_DIR}/eval_results.json}"
WANDB_PROJECT="${WANDB_PROJECT:-slimpajama-continued}"
WANDB_RUN="${WANDB_RUN:-gemma-scratch-eval-$(date +%Y%m%d-%H%M)}"
